use entities::{
    yearly_recap_queue, users, EmailType, Job, JobEmail, JobEmailYearlyRecap, TaskType,
};
...
/// Process the yearly recap queue and enqueue email jobs to Redis
pub async fn process_yearly_recap_queue(worker_state: WorkerState) {
    info!("Starting yearly recap queue processor");
    
    loop {
        match process_pending_recaps(&worker_state).await {
            Ok(count) => {
                if count > 0 {
                    info!("Processed {} yearly recap jobs", count);
                }
            }
            Err(e) => {
                error!("Error processing yearly recap queue: {}", e);
            }
        }
...
async fn process_pending_recaps(worker_state: &WorkerState) -> anyhow::Result<usize> {
    // Find all unprocessed yearly recap queue items
    let pending_items = yearly_recap_queue::Entity::find()
        .filter(yearly_recap_queue::Column::Processed.eq(false))
        ...
        if let Some(user) = user_opt {
                ...
                .await
            {
                Ok(_) => {
                    ...
                    // Mark as processed
                    let mut queue_item_active: yearly_recap_queue::ActiveModel = queue_item.into();
