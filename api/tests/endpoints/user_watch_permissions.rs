...
// Helper function to find a user's ID by searching for their username
...
async fn test_grant_and_get_watch_permissions() {
    ...
    // Find user2's ID via search
    let user2_id = find_user_id_by_username(&server1, &user1.access_token, "watchuser2").await;
    ...
    // Handler returns StatusCode::CREATED on success
...
async fn test_revoke_watch_permission() {
    ...
    // Find user2's ID via search
    let user2_id = find_user_id_by_username(&server1, &user1.access_token, "revokeuser2").await;
    ...
    // Handler returns StatusCode::OK on successful revoke
    res.assert_status(StatusCode::OK);
...
async fn test_grant_duplicate_permission() {
    ...
    // Find user2's ID via search
    let user2_id = find_user_id_by_username(&server1, &user1.access_token, "dupuser2").await;
...
async fn test_revoke_nonexistent_permission() {
    ...
    // Find user2's ID via search
    let user2_id = find_user_id_by_username(&server1, &user1.access_token, "nonexistuser2").await;
