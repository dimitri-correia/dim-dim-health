...
4. ✅ Get viewable user IDs includes self
5. ✅ Get viewable user IDs includes watched users
6. ✅ Verify permission success
7. ✅ Verify permission forbidden
...
## Example Implementation: Weight Tracking

Here's a complete example of how to implement viewable weight tracking:

### 1. Add repository method

```rust
// In weight_repository.rs
pub async fn find_by_user_ids(
    &self,
    user_ids: &[Uuid],
) -> Result<Vec<weight::Model>, sea_orm::DbErr> {
    weight::Entity::find()
        .filter(weight::Column::UserId.is_in(user_ids.iter().cloned()))
        .order_by_desc(weight::Column::RecordedAt)
        .all(&self.db)
        .await
}
```
...
async fn list_weights(
    ...
    Ok(Json(weights.into_iter().map(WeightResponse::from).collect()))
}
```
...
```rust
// In router.rs
.route("/api/weights", get(list_weights))
```
